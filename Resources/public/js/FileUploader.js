function PunkAveFileUploader(options)
{
	...
	self.initCallback = 'initCallback' in options ? options.initCallback : function ( ) {
	};
	self.initCallback();
	...
	self.transformTemplateFunction = 'transformTemplateFunction' in options ? options.transformTemplateFunction : function (data) {
		return data;
	};
	...
	function appendEditableImage(info)
	{
		...
		var html = fileTemplate(info);
		var li = $(self.transformTemplateFunction(html, info));
